...
for times in tqdm(range(total_start_time, total_end_time, step_size)):
    ...
    fig1.suptitle(f"Range: {tfmt1} to {tfmt2} (GMT)")
    fig2.suptitle(f"Range: {tfmt1} to {tfmt2} (GMT)")
    fig3.suptitle(f"Range: {tfmt1} to {tfmt2} (GMT)")

    fig1.savefig(f"/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/xy/tof_xy_{times}_{event_counter}.png")
    fig2.savefig(f"/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cube_sides/tof_cube_sides_{times}_{event_counter}.png")
    fig3.savefig(f"/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cortina/tof_cortina_{times}_{event_counter}.png")
...
for filename in sorted(glob.glob("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cortina/*.png")):
    im = Image.open(filename)
    cor_images.append(im)

cor_images[0].save("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cor.gif", save_all=True, append_images=cor_images[1:], duration=1000, loop=0) # duration - duration of each frame in GIF in ms, loop - 0 means loop forever

xy_images = []
print("making xy gif")
for filename in sorted(glob.glob("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/xy/*.png")):
    im = Image.open(filename)
    xy_images.append(im)

xy_images[0].save("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/xy.gif", save_all=True, append_images=xy_images[1:], duration=1000, loop=0) # duration - duration of each frame in GIF in ms, loop - 0 means loop forever

cube_images = []
print("making xy gif")
for filename in sorted(glob.glob("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cube_sides/*.png")):
    im = Image.open(filename)
    cube_images.append(im)

cube_images[0].save("/home/suraj.a/gaps_tof/images/full_flight_TOF_occupancy_hourly/cube_sides.gif", save_all=True, append_images=cube_images[1:], duration=1000, loop=0) # duration - duration of each frame in GIF in ms, loop - 0 means loop forever